...
const createPrediction = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!chatflow) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Chatflow ${req.params.id} not found`)
        }
        ...
        if (isDomainAllowed) {
            ...
            throw new InternalAgentForgeError(StatusCodes.FORBIDDEN, unauthorizedOriginError)
        }
    } catch (error) {
        next(error)
    }
