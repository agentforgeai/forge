...
import { ICommonObject, INodeData } from 'AgentForge-components'
import { convertToOpenAIFunction } from '@langchain/core/utils/function_calling'
import { v4 as uuidv4 } from 'uuid'

const SOURCE_DOCUMENTS_PREFIX = '\n\n----AgentForge_SOURCE_DOCUMENTS----\n\n'
const ARTIFACTS_PREFIX = '\n\n----AgentForge_ARTIFACTS----\n\n'
...
const buildAndInitTool = async (chatflowid: string, _chatId?: string, _apiMessageId?: string) => {
    ...
    if (!toolAgentNode) {
        throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Agent with tools not found in chatflow ${chatflowid}`)
    }
...
const executeAgentTool = async (
    ...
    try {
        ...
        if (!tool) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Tool ${toolName} not found`)
        }
