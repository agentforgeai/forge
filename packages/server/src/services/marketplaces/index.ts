...
const getAllTemplates = async () => {
    try {
        ...
        const AgentForgeDocsQnAIndex = sortedTemplates.findIndex((tmp) => tmp.templateName === 'AgentForge Docs QnA')
        if (AgentForgeDocsQnAIndex > 0) {
            sortedTemplates.unshift(sortedTemplates.splice(AgentForgeDocsQnAIndex, 1)[0])
        }
