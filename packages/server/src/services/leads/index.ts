...
const getAllLeads = async (chatflowid: string) => {
    try {
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: leadsService.getAllLeads - ${getErrorMessage(error)}`)
    }
}
...
const createLead = async (body: Partial<ILead>) => {
    try {
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: leadsService.createLead - ${getErrorMessage(error)}`)
    }
}
