...
const createAssistant = async (requestBody: any): Promise<Assistant> => {
    try {
        ...
        if (!requestBody.details) {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Invalid request body`)
        }
        ...
        try {
            ...
            if (!credential) {
                throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Credential ${requestBody.credential} not found`)
            }
            ...
            if (!openAIApiKey) {
                throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `OpenAI ApiKey not found`)
            }
            ...
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error creating new assistant - ${getErrorMessage(error)}`)
        }
        ...
        appServer.metricsProvider?.incrementCounter(AgentForge_METRIC_COUNTERS.ASSISTANT_CREATED, { status: AgentForge_COUNTER_STATUS.SUCCESS })
        return dbResponse
    } catch (error) {
...
const deleteAssistant = async (assistantId: string, isDeleteBoth: any): Promise<DeleteResult> => {
    try {
        ...
        if (!assistant) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Assistant ${assistantId} not found`)
        }
        try {
            ...
            if (!credential) {
                throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Credential ${assistant.credential} not found`)
            }
            ...
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error deleting assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new InternalAgentForgeError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const updateAssistant = async (assistantId: string, requestBody: any): Promise<Assistant> => {
    try {
        ...
        try {
            ...
            if (!credential) {
                throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Credential ${body.credential} not found`)
            }
            ...
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error updating assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new InternalAgentForgeError(
            StatusCodes.INTERNAL_SERVER_ERROR,
