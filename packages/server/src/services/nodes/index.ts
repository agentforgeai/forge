...
import { INodeOptionsValue, ICommonObject, handleEscapeCharacters } from 'AgentForge-components'
...
const getAllNodes = async () => {
    try {
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: nodesService.getAllNodes - ${getErrorMessage(error)}`)
    }
}
...
const getNodeByName = async (nodeName: string) => {
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(appServer.nodesPool.componentNodes, nodeName)) {
            ...
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Node ${nodeName} not found`)
        }
    } catch (error) {
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: nodesService.getAllNodes - ${getErrorMessage(error)}`)
    }
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(appServer.nodesPool.componentNodes, nodeName)) {
            ...
            if (nodeInstance.icon === undefined) {
                throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Node ${nodeName} icon not found`)
            }
            ...
            if (nodeInstance.icon.endsWith('.svg') || nodeInstance.icon.endsWith('.png') || nodeInstance.icon.endsWith('.jpg')) {
                ...
                throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Node ${nodeName} icon is missing icon`)
            }
        } else {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Node ${nodeName} not found`)
        }
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(appServer.nodesPool.componentNodes, 'customFunction')) {
            try {
                ...
                throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error running custom function: ${error}`)
            }
        } else {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Node customFunction not found`)
        }
    } catch (error) {
        throw new InternalAgentForgeError(
            StatusCodes.INTERNAL_SERVER_ERROR,
