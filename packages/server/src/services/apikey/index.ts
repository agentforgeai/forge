...
const getAllApiKeys = async () => {
    try {
        if (_apikeysStoredInJson()) {
            ...
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `UNKNOWN APIKEY_STORAGE_TYPE`)
        }
    } catch (error) {
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.getAllApiKeys - ${getErrorMessage(error)}`)
    }
}
...
const getApiKey = async (apiKey: string) => {
    try {
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.createApiKey - ${getErrorMessage(error)}`)
    }
}
...
const updateApiKey = async (id: string, keyName: string) => {
    try {
        if (_apikeysStoredInJson()) {
            ...
            if (!currentKey) {
                throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `ApiKey ${currentKey} not found`)
            }
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.updateApiKey - ${getErrorMessage(error)}`)
    }
}
...
const deleteApiKey = async (id: string) => {
    try {
        if (_apikeysStoredInJson()) {
            ...
            if (!dbResponse) {
                throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `ApiKey ${id} not found`)
            }
            return getAllApiKeys()
        } else {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `UNKNOWN APIKEY_STORAGE_TYPE`)
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.deleteApiKey - ${getErrorMessage(error)}`)
    }
}
...
const importKeys = async (body: any) => {
    try {
        ...
        if (splitDataURI[0] !== 'data:application/json;base64') {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Invalid dataURI`)
        }
        ...
        if (_apikeysStoredInJson()) {
            ...
            if (body.importMode === 'errorIfExist') {
                ...
                for (const key of keys) {
                    ...
                    if (keyNameExists) {
                        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Key with name ${key.keyName} already exists`)
                    }
                }
            }
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: apikeyService.importKeys - ${getErrorMessage(error)}`)
    }
}
...
const verifyApiKey = async (paramApiKey: string): Promise<string> => {
    try {
        if (_apikeysStoredInJson()) {
            ...
            if (!apiKey) {
                throw new InternalAgentForgeError(StatusCodes.UNAUTHORIZED, `Unauthorized`)
            }
            return 'OK'
        } else if (_apikeysStoredInDb()) {
            const appServer = getRunningExpressApp()
        ...
        if (error instanceof InternalAgentForgeError && error.statusCode === StatusCodes.UNAUTHORIZED) {
            throw error
        } else {
