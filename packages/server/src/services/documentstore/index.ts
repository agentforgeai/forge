...
import { AgentForge_COUNTER_STATUS, AgentForge_METRIC_COUNTERS } from '../../Interface.Metrics'
...
const deleteLoaderFromDocumentStore = async (storeId: string, loaderId: string) => {
    try {
        ...
        if (found) {
            ...
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Unable to locate loader in Document Store ${entity.name}`)
        }
    } catch (error) {
        throw new InternalAgentForgeError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const getDocumentStoreFileChunks = async (storeId: string, fileId: string, pageNo: number = 1) => {
    try {
        ...
        if (!chunksWithCount) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `File ${fileId} not found`)
        }
...
const deleteDocumentStore = async (storeId: string) => {
    try {
        ...
        if (!entity) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Document store ${storeId} not found`)
        }
...
const deleteDocumentStoreFileChunk = async (storeId: string, docId: string, chunkId: string) => {
    try {
        ...
        if (!found) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Document store loader ${docId} not found`)
        }
        ...
        if (!tbdChunk) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Document Chunk ${chunkId} not found`)
        }
...
const deleteVectorStoreFromStore = async (storeId: string) => {
    try {
        ...
        if (!entity.embeddingConfig) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Embedding for Document store ${storeId} not found`)
        }

        if (!entity.vectorStoreConfig) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Vector Store for Document store ${storeId} not found`)
        }
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (input.startsWith('FILE-STORAGE::')) {
            if (!documentStoreEntity) {
                ...
                if (!documentStoreEntity) {
                    throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Document store ${data.storeId} not found`)
                }
            }
...
const _insertIntoVectorStoreWorkerThread = async (data: ICommonObject) => {
    try {
        ...
        appServer.metricsProvider?.incrementCounter(AgentForge_METRIC_COUNTERS.VECTORSTORE_UPSERT, { status: AgentForge_COUNTER_STATUS.SUCCESS })
...
const queryVectorStore = async (data: ICommonObject) => {
    try {
        ...
        if (!entity) {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Document store ${data.storeId} not found`)
        }
        const options: ICommonObject = {
            chatflowid: uuidv4(),
            appDataSource: appServer.AppDataSource,
        ...
        if (!entity.embeddingConfig) {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Embedding for ${data.storeId} is not configured`)
        }

        if (!entity.vectorStoreConfig) {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Vector Store for ${data.storeId} is not configured`)
        }
        ...
        if (!retriever) {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create retriever`)
        }
        const startMillis = Date.now()
        const results = await retriever.invoke(data.query, undefined)
        if (!results) {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to retrieve results`)
        }
...
const _createEmbeddingsObject = async (
    ...
    if (!embeddingObj) {
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create EmbeddingObj`)
    }
    return embeddingObj
}
...
const _createRecordManagerObject = async (
    ...
    if (!recordManagerObj) {
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create RecordManager obj`)
    }
