import { removeFolderFromStorage } from 'AgentForge-components'
...
const getChatflowByApiKey = async (apiKeyId: string, keyonly?: unknown): Promise<any> => {
    try {
        ...
        if (dbResponse.length < 1) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Chatflow not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalAgentForgeError(
...
const saveChatflow = async (newChatFlow: ChatFlow): Promise<any> => {
    try {
        ...
        appServer.metricsProvider?.incrementCounter(
            dbResponse?.type === 'MULTIAGENT' ? AgentForge_METRIC_COUNTERS.AGENTFLOW_CREATED : AgentForge_METRIC_COUNTERS.CHATFLOW_CREATED,
    ...
    try {
        ...
        if (dbResponse.chatbotConfig || uploadsConfig) {
            try {
                ...
                throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error parsing Chatbot Config for Chatflow ${chatflowId}`)
            }
        }
        return 'OK'
    } catch (error) {
