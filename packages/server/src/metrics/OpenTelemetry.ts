import { AgentForge_METRIC_COUNTERS, IMetricsProvider } from '../Interface.Metrics'
...
import { getVersion } from 'AgentForge-components'
...
export class OpenTelemetry implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        ...
        const AgentForgeVersion = await getVersion()

        this.resource = new Resource({
            [ATTR_SERVICE_NAME]: process.env.METRICS_SERVICE_NAME || 'AgentForgeAI',
            [ATTR_SERVICE_VERSION]: AgentForgeVersion.version // Version as a label
        })
        ...
        const meter = this.meterProvider.getMeter('AgentForge-metrics')
        // look at the AgentForge_COUNTER enum in Interface.Metrics.ts and get all values
        // for each counter in the enum, create a new promClient.Counter and add it to the registry
        const enumEntries = Object.entries(AgentForge_METRIC_COUNTERS)
        ...
        const versionGuage = meter.createGauge('AgentForge_version', {
            description: 'AgentForge version'
        })
        // remove the last dot from the version string, e.g. 2.1.3 -> 2.13 (guage needs a number - float)
        const formattedVersion = AgentForgeVersion.version.replace(/\.(\d+)$/, '$1')
