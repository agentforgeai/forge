...
export class Prometheus implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        const serviceName: string = process.env.METRICS_SERVICE_NAME || 'AgentForgeAI'
        ...
        const versionGaugeCounter = new promClient.Gauge({
            name: 'AgentForge_version_info',
            help: 'AgentForge version info.',
            labelNames: ['version']
        })

        const { version } = await getVersion()
        versionGaugeCounter.set({ version: 'v' + version }, 1)
        this.counters.set('AgentForge_version', versionGaugeCounter)
    ...
    public incrementCounter(counter: AgentForge_METRIC_COUNTERS, payload: any) {
