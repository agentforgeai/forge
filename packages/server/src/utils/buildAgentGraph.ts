...
export const buildAgentGraph = async (
    ...
    try {
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error buildAgentGraph - ${getErrorMessage(e)}`)
    }
}
...
const compileMultiAgentsGraph = async (params: MultiAgentsGraphParams) => {
    ...
    for (const workerNode of workerNodes) {
        ...
        try {
            ...
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error initialize worker nodes - ${getErrorMessage(e)}`)
        }
    }
    ...
    for (const supervisor in supervisorWorkers) {
        ...
        try {
            ...
            //         }
            //     } catch (e) {
            //         throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, getErrorMessage(e))
            //     }
            // }
            ...
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error initialize supervisor nodes - ${getErrorMessage(e)}`)
        }
    }
}
...
const compileSeqAgentsGraph = async (params: SeqAgentsGraphParams) => {
    ...
    if (!startAgentNodes.length) throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, 'Start node not found')
    if (startAgentNodes.length > 1)
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, 'Graph should have only one start node')
    ...
    if (!endAgentNodes.length && !loopNodes.length) {
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, 'Graph should have at least one End/Loop node')
    }
    /*** End of Validation ***/
    ...
    for (const agentNodeId of getSortedDepthNodes(depthQueue)) {
        ...
        if (eligibleSeqNodes.includes(agentNode.data.name)) {
            try {
                ...
                throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error initialize agent nodes - ${getErrorMessage(e)}`)
            }
        }
    }
    ...
    try {
        ...
        throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Error compile graph - ${getErrorMessage(e)}`)
    }
}
