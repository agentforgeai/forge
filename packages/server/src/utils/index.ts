...
const REDACTED_CREDENTIAL_VALUE = '_AgentForge_BLANK_07167752-1a71-43b1-bf8f-4f32252165db'
...
export const getEndingNodes = (
    ...
    let error: InternalAgentForgeError | null = null
    for (const endingNode of endingNodes) {
        const endingNodeData = endingNode.data
        if (!endingNodeData) {
            error = new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending node ${endingNode.id} data not found`)
        ...
        if (!isEndingNode) {
            if (
                ...
                error = new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending node must be either a Chain or Agent or Engine`)
                continue
            }
        }
    ...
    if (endingNodes.length === 0 || error === null) {
        error = new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending nodes not found`)
    }
    ...
    for (const node of reactFlowNodes) {
        ...
        if (sessionId && node.data.inputs) {
            if (node.data.type === 'OpenAIAssistant') {
                ...
                const initializedInstance: AgentForgeMemory = await newNodeInstance.init(node.data, '', options)
...
export const getVariableValue = async (
    ...
    while (startIdx < endIdx) {
        ...
        if (substr === '}}' && variableStack.length > 0 && variableStack[variableStack.length - 1].substr === '{{') {
             ...
             * FROM: hello i am ben\n\n\thow are you?
             * TO: hello i am benAgentForge_NEWLINEAgentForge_NEWLINEAgentForge_TABhow are you?
             */
...
export const getEncryptionKey = async (): Promise<string> => {
    if (process.env.AgentForge_SECRETKEY_OVERWRITE !== undefined && process.env.AgentForge_SECRETKEY_OVERWRITE !== '') {
        return process.env.AgentForge_SECRETKEY_OVERWRITE
    }
    try {
        ...
        const defaultLocation = process.env.SECRETKEY_PATH
            ...
            : path.join(getUserHome(), '.AgentForge', 'encryption.key')
...
export const getSessionChatHistory = async (
    ...
    const initializedInstance: AgentForgeMemory = await newNodeInstance.init(memoryNode.data, '', {
        chatflowid,
        appDataSource,
        databaseEntities,
        logger
...
export const getUserSettingsFilePath = () => {
    ...
    const checkPaths = [path.join(getUserHome(), '.AgentForge', 'settings.json')]
    for (const checkPath of checkPaths) {
        if (fs.existsSync(checkPath)) {
            return checkPath
        }
