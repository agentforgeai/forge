...
import { IAction } from 'AgentForge-components'
...
export const utilBuildChatflow = async (req: Request, isInternal: boolean = false): Promise<any> => {
    ...
    try {
        ...
        if (isFlowReusable()) {
            ...
            if (!nodeToExecute) {
                throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Node not found`)
            }
        ...
        appServer.metricsProvider?.incrementCounter(
            isInternal ? AgentForge_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : AgentForge_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL,
            { status: AgentForge_COUNTER_STATUS.SUCCESS }
        )
        // Prepare response
        ...
        appServer.metricsProvider?.incrementCounter(
            isInternal ? AgentForge_METRIC_COUNTERS.CHATFLOW_PREDICTION_INTERNAL : AgentForge_METRIC_COUNTERS.CHATFLOW_PREDICTION_EXTERNAL,
            { status: AgentForge_COUNTER_STATUS.FAILURE }
        )
...
const utilBuildAgentResponse = async (
    ...
    try {
        ...
        if (streamResults) {
            ...
            appServer.metricsProvider?.incrementCounter(
                isInternal ? AgentForge_METRIC_COUNTERS.AGENTFLOW_PREDICTION_INTERNAL : AgentForge_METRIC_COUNTERS.AGENTFLOW_PREDICTION_EXTERNAL,
                { status: AgentForge_COUNTER_STATUS.SUCCESS }
            )
