...
import { ICommonObject, IMessage, addArrayFilesToStorage, mapMimeTypeToInputField, mapExtToInputField } from 'AgentForge-components'
...
export const upsertVector = async (req: Request, isInternal: boolean = false) => {
    try {
        ...
        if (!chatflow) {
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowid} not found`)
        }
        ...
        if (vsNodesWithFileUpload.length > 1) {
            throw new InternalAgentForgeError(StatusCodes.INTERNAL_SERVER_ERROR, 'Multiple vector store nodes with fileUpload enabled')
        ...
        if (vsNodes.length > 1 && !stopNodeId) {
            ...
            throw new InternalAgentForgeError(StatusCodes.NOT_FOUND, 'No vector node found')
        }
        ...
        if (e instanceof InternalAgentForgeError && e.statusCode === StatusCodes.UNAUTHORIZED) {
            throw e
        } else {
